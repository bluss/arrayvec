...
fn test_splice_works_when_it_should() {
    ...
    // Typical case
    let vec_popped: Vec<_> = vec.splice( 1..4, [11, 12, 13]).into_iter().collect();
    assert_eq!(&vec[..], &[0, 11, 12, 13, 4]);
    assert_eq!(&vec_popped, &[1, 2, 3]);
    let array_popped: Vec<_> = array.splice( 1..4, [11, 12, 13]).into_iter().collect();
    assert_eq!(&array[..], &vec[..]);
    assert_eq!(&array_popped, &vec_popped);
    // `replace_with` shorter than `range`
    let vec_popped: Vec<_> = vec.splice( 2..5, [21, 22]).into_iter().collect();
    assert_eq!(&vec[..], &[0, 11, 21, 22]);
    assert_eq!(&vec_popped, &[12, 13, 4]);
    let array_popped: Vec<_> = array.splice( 2..5, [21, 22]).into_iter().collect();
    assert_eq!(&array[..], &vec[..]);
    assert_eq!(&array_popped, &vec_popped);
    // `range` shorter than `replace_with`
    let vec_popped: Vec<_> = vec.splice( 3..4, [31, 32]).into_iter().collect();
    assert_eq!(&vec[..], &[0, 11, 21, 31, 32]);
    assert_eq!(&vec_popped, &[22]);
    let array_popped: Vec<_> = array.splice( 3..4, [31, 32]).into_iter().collect();
    assert_eq!(&array[..], &vec[..]);
    assert_eq!(&array_popped, &vec_popped);
    //`replace_with` shorter than open `range`
    let vec_popped: Vec<_> = vec.splice( 1.., [41, 42]).into_iter().collect();
    assert_eq!(&vec[..], &[0, 41, 42]);
    assert_eq!(&vec_popped, &[11, 21, 31, 32]);
    let array_popped: Vec<_> = array.splice( 1.., [41, 42]).into_iter().collect();
    assert_eq!(&array[..], &vec[..]);
    assert_eq!(&array_popped, &vec_popped);
    // `range` shorter than open `replace_with`
    let vec_popped: Vec<_> = vec.splice( 3.., [51, 52]).into_iter().collect();
    assert_eq!(&vec[..], &[0, 41, 42, 51, 52]);
    assert_eq!(&vec_popped, &[]);
    let array_popped: Vec<_> = array.splice( 3.., [51, 52]).into_iter().collect();
    assert_eq!(&array[..], &vec[..]);
    assert_eq!(&array_popped, &vec_popped);
}
...
fn test_splice_fails_on_overflowing_range() {
    ...
    let mut vec: Vec<_> = (0..5).collect();
    let mut array: ArrayVec<_,  5> = (0..5).collect();
    let vec_popped: Vec<_> = vec.splice( 4..6, [11]).into_iter().collect();
