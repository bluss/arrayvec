...
macro_rules! main_impl {
    ( $array:ident $(, $bound:ident$(, $const:ident)?)?) => {
        impl<T $(: $bound)*, const CAP: usize> $array<T, CAP> {
            /// Capacity
            ...
            pub fn new() -> $array<T, CAP> {
                ...
                unsafe {
                    $array { xs: MaybeUninit::uninit().assume_init(), len: 0 }
                }
            }
            ...
            /// use arrayvec::$array;
            ///
            /// let mut array = $array::from([1, 2, 3, 4, 5]);
            /// array.truncate(3);
            ...
            pub(crate) unsafe fn get_unchecked_ptr(&mut self, index: usize) -> *mut T {
                ...
                where F: FnMut(&mut T) -> bool
            {
                ...
                struct BackshiftOnDrop<'a, T $(: $bound)*, const CAP: usize> {
                    v: &'a mut $array<T, CAP>,
                    processed_len: usize,
                    deleted_cnt: usize,
                    original_len: usize,
                ...
                impl<T $(: $bound)*, const CAP: usize> Drop for BackshiftOnDrop<'_, T, CAP> {
                    fn drop(&mut self) {
                ...
                #[inline(always)]
                fn process_one<F: FnMut(&mut T) -> bool, T $(: $bound)*, const CAP: usize, const DELETED: bool>(
                    f: &mut F,
...
macro_rules! deref_impl {
    ( $array:ident $(, $bound:ident)?) => {
        ...
        impl<T, const CAP: usize> DerefMut for $array<T, CAP>
            where $(T: $bound)*
        {
            #[inline]
...
macro_rules! conversion_impl {
    ( $array:ident $(, $bound:ident)?) => {
        ...
        /// assert_eq!(array.capacity(), 3);
        /// ```
        impl<T, const CAP: usize> From<[T; CAP]> for $array<T, CAP>
            where $(T: $bound)*
        {
            fn from(array: [T; CAP]) -> Self {
                let array = ManuallyDrop::new(array);
                let mut vec = <$array<T, CAP>>::new();
                unsafe {
        ...
        /// ```
        impl<T, const CAP: usize> std::convert::TryFrom<&[T]> for $array<T, CAP>
            where T: Clone $(+ $bound)*
        {
...
#[cfg(feature="copy")]
conversion_impl!(ArrayVecCopy, Copy);
conversion_impl!(ArrayVec);
...
macro_rules! iterator_impl {
    ( $array:ident $(, $bound:ident)?) => {
        ...
        /// }
        /// ```
        impl<'a, T: 'a, const CAP: usize> IntoIterator for &'a $array<T, CAP>
            where $(T: $bound)*
        {
            type Item = &'a T;
        ...
        /// }
        /// ```
        impl<'a, T: 'a, const CAP: usize> IntoIterator for &'a mut $array<T, CAP>
            where $(T: $bound)*
        {
            type Item = &'a mut T;
...
#[cfg(feature="copy")]
iterator_impl!(ArrayVecCopy, Copy);
iterator_impl!(ArrayVec);
...
macro_rules! extend_impl {
    ( $array:ident $(, $bound:ident)?) => {
        ...
        impl<T, const CAP: usize> Extend<T> for $array<T, CAP>
            where $(T: $bound)*
        {
            /// Extend the `$array` with an iterator.
            /// 
...
#[cfg(feature="copy")]
extend_impl!(ArrayVecCopy, Copy);
extend_impl!(ArrayVec);
...
macro_rules! from_iter_impl {
    ( $array:ident $(, $bound:ident)?) => {
        ...
        impl<T, const CAP: usize> iter::FromIterator<T> for $array<T, CAP>
            where $(T: $bound)*
        {
            /// Create an `$array` from an iterator.
            /// 
...
#[cfg(feature="copy")]
from_iter_impl!(ArrayVecCopy, Copy);
from_iter_impl!(ArrayVec);

macro_rules! clone_impl {
    ( $array:ident $(, $bound:ident)?) => {
        impl<T, const CAP: usize> Clone for $array<T, CAP>
            where T: Clone $(+ $bound)*
        {
            fn clone(&self) -> Self {
...
#[cfg(feature="copy")]
clone_impl!(ArrayVecCopy, Copy);
clone_impl!(ArrayVec);

macro_rules! compare_impl {
    ( $array:ident $(, $bound:ident)?) => {
        impl<T, const CAP: usize> Hash for $array<T, CAP>
            where T: Hash $(+ $bound)*
        {
        ...
        impl<T, const CAP: usize> PartialEq for $array<T, CAP>
            where T: PartialEq $(+ $bound)*
        {
        ...
        impl<T, const CAP: usize> PartialEq<[T]> for $array<T, CAP>
            where T: PartialEq $(+ $bound)*
        {
        ...
        impl<T, const CAP: usize> Eq for $array<T, CAP> where T: Eq $(+ $bound)* { }

        impl<T, const CAP: usize> PartialOrd for $array<T, CAP>
        where
            T: PartialOrd $(+ $bound)*
        {
        ...
        impl<T, const CAP: usize> Ord for $array<T, CAP>
        where
            T: Ord $(+ $bound)*
        {
...
#[cfg(feature="copy")]
compare_impl!(ArrayVecCopy, Copy);
compare_impl!(ArrayVec);

macro_rules! borrowing_impl {
    ( $array:ident $(, $bound:ident)?) => {
        impl<T $(: $bound)*, const CAP: usize> Borrow<[T]> for $array<T, CAP> {
            fn borrow(&self) -> &[T] { self }
        }

        impl<T $(: $bound)*, const CAP: usize> BorrowMut<[T]> for $array<T, CAP> {
            fn borrow_mut(&mut self) -> &mut [T] { self }
        }

        impl<T $(: $bound)*, const CAP: usize> AsRef<[T]> for $array<T, CAP> {
            fn as_ref(&self) -> &[T] { self }
        }

        impl<T $(: $bound)*, const CAP: usize> AsMut<[T]> for $array<T, CAP> {
            fn as_mut(&mut self) -> &mut [T] { self }
        }
    }
}
#[cfg(feature="copy")]
borrowing_impl!(ArrayVecCopy, Copy);
borrowing_impl!(ArrayVec);
...
macro_rules! serde_impl {
    ( $array:ident $(, $bound:ident)?) => {
        ...
        impl<T: Serialize $(+ $bound)*, const CAP: usize> Serialize for $array<T, CAP> {
        ...
        /// Requires crate feature `"serde"`
        impl<'de, T: Deserialize<'de> $(+ $bound)*, const CAP: usize> Deserialize<'de> for $array<T, CAP> {
                ...
                where D: Deserializer<'de>
            {
                ...
                impl<'de, T: Deserialize<'de> $(+ $bound)*, const CAP: usize> Visitor<'de> for ArrayVecVisitor<'de, T, CAP> {
                    type Value = $array<T, CAP>;
                    ...
                    {
                        let mut values = $array::<T, CAP>::new();
...
#[cfg(all(feature="serde", feature="copy"))]
serde_impl!(ArrayVecCopy, Copy);
#[cfg(feature="serde")]
serde_impl!(ArrayVec);
